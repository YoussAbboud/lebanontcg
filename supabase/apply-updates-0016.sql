...
-- Applies only migration 0016 (the admin console: roles, suspensions, the
-- report workflow and the audit log). Use this when the project already
-- runs 0001–0015.
--
-- HOW TO USE: Supabase dashboard → SQL Editor → New query → paste this
-- entire file → Run. Run it ONCE.
--
-- REQUIRED afterwards — promote your first admin (there is deliberately no
-- way to become one through the app):
--
--   select set_config('app.admin_action', '1', true);
--   update public.profiles set is_admin = true
--     where id = (select id from auth.users where email = 'you@example.com');
--
-- After that, admins promote each other from /admin → Users.
-- ==========================================================================
